...
public class ServiceImpl implements ServiceIfc{
	...
	private final Map<String,Map<String,List<String>>> masterSearchTicketMapGroupedByAttributes = 
			new HashMap<String, Map<String,List<String>>>();

	private final Map<String,Map<String,List<String>>> masterSearchUserMapGroupedByAttributes = 
			new HashMap<String, Map<String,List<String>>>();
	...
	public List<Ticket> getTicketDisplayResults(String key,String value) {
		...
		Map<String, Ticket> masterTicketMapGroupById =
		...
		Map<String, User> masterUserMapGroupById = masterUserList.stream().collect(Collectors.toMap(User::getId,
				Function.identity()));

		/** finally store all these maps into a master map for Tickets **/
		//Map<String,Map<String,List<String>>> masterSearchTicketMapGroupedByAttributes = new HashMap<String, Map<String,List<String>>>();
		Map<String,Map<String,List<String>>> masterSearchGroupedByAttributesMap = groupTicketDataByAttributes(masterTicketList);
		

		/** #1 using search key extract the map that needs to be searched  */
		Map<String,List<String>> mapToSearch = masterSearchGroupedByAttributesMap.get(key);

		/** #2 using the search value extract from this map List of Ticket Ids **/
		List<String> ticketIdSearchResultList = mapToSearch.get(value);

		/** #3 we may or may not get any search results **/
		if(null != ticketIdSearchResultList && ticketIdSearchResultList.size() > 0) {
			 ...
			 *      return results
			 **/
			ticketIdSearchResultList.forEach((ticketId) -> {
				Ticket ticket = masterTicketMapGroupById.get(ticketId);
				/** now get the assigneeId - this can be null / empty **/
				if(!StringUtils.isBlank(ticket.getAssigneeId())){
					/** to populate assignee name **/
					User user = masterUserMapGroupById.get(ticket.getAssigneeId());
					if(null != user) {
	...
	public List<Ticket> getTicketDisplayResults(String value) {
		...
		/** generate or get 'masterTicketMap' from List based on ticketId **/ 
		Map<String, Ticket> masterTicketMapGroupById =
				masterTicketList.stream().collect(Collectors.toMap(Ticket::getId,
						Function.identity()));
		...
		Ticket ticket = masterTicketMapGroupById.get(value);
	...
	public List<User> getUserDisplayResults(String key,String value) {
		...
		Map<String,Map<String,List<String>>> masterSearchByUserMap = groupUserDataByAttributes(masterUserList);
		/**
	...
	private Map<String,Map<String,List<String>>> groupTicketDataByAttributes(List<Ticket> masterTicketList) {
		/** if the map already has the data simply return that data  if map is empty
		 *  the data has not been arranged lets do that
		 *  finally store all these maps into a master map for Tickets
		 *  and return the loaded map
		 **/
		if(null == masterSearchTicketMapGroupedByAttributes || masterSearchTicketMapGroupedByAttributes.size() <= 0) {
			/** load data **/
			populateMapWithTicketDataGroupedByAttributes(masterTicketList);
		}
		return masterSearchTicketMapGroupedByAttributes;		
	}

	private Map<String,Map<String,List<String>>> groupUserDataByAttributes(List<User> masterUserList) {
		/** if the map already has the data simply return that data  if map is empty
		 *  the data has not been arranged lets do that
		 *  finally store all these maps into a master map for Tickets
		 *  and return the loaded map
		 **/
		if(null == masterSearchUserMapGroupedByAttributes || masterSearchUserMapGroupedByAttributes.size() <= 0) {
			/** load data **/
			populateMapWithUserDataGroupedByAttributes(masterUserList);
		}
		return masterSearchUserMapGroupedByAttributes;		
	}


	private void populateMapWithTicketDataGroupedByAttributes(List<Ticket> masterTicketList) {		
		...
		masterSearchTicketMapGroupedByAttributes.put("created_at", ticketMapByCreatedDate);
		masterSearchTicketMapGroupedByAttributes.put("type", ticketMapByType);
		masterSearchTicketMapGroupedByAttributes.put("subject", ticketMapBySubject);
		masterSearchTicketMapGroupedByAttributes.put("assignee_id", ticketMapByAssignee);
		masterSearchTicketMapGroupedByAttributes.put("tags", ticketMapByTags);

	}

	private void populateMapWithUserDataGroupedByAttributes(List<User> masterUserList) {
		...
		/** finally store all these maps into a master map for Users **/
		masterSearchUserMapGroupedByAttributes.put("name", userMapByName);
		masterSearchUserMapGroupedByAttributes.put("created_at", userMapByCreationAt);
		masterSearchUserMapGroupedByAttributes.put("verified", userMapByVerification);
